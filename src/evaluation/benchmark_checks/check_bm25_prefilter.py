...
def _call_retrieval(api_base: str, query: str, top_k: int,
                    use_bm25_prefilter: bool) -> list[str]:
...
def _evaluate(gold, ks: list[int], api_base: str,
              use_bm25_prefilter: bool, label: str) -> dict[str, float]:
    max_k = max(ks)
    total_rel = 1
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="src/evaluation/results/bm25_prefilter_check")
    ...
    print("=" * 60)
    summary_a = _evaluate(gold, ks, args.api_base_url, use_bm25_prefilter=False, label="baseline")

    print("\n" + "=" * 60)
    print("RUN 2/2 — BM25 keyword pre-filter → cross-encoder")
    print("=" * 60)
    summary_b = _evaluate(gold, ks, args.api_base_url, use_bm25_prefilter=True, label="bm25_pre")
    ...
    if args.check_coverage:
        ...
        if gold_covered:
            ...
            print("=" * 60)
            cch_a = _evaluate(gold_covered, ks, args.api_base_url, use_bm25_prefilter=False, label="cch_baseline")
            print("\n" + "=" * 60)
            print("CCH RUN 2/2 — bm25_prefilter (covered only)")
            print("=" * 60)
            cch_b = _evaluate(gold_covered, ks, args.api_base_url, use_bm25_prefilter=True, label="cch_bm25_pre")
            print("\n\n===== CCH COMPARISON: baseline vs bm25_prefilter (covered only) =====")
            cch_comparison = _print_comparison(cch_a, cch_b, "cch_baseline", "cch_bm25_pre")
