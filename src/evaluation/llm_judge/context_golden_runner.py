"""
Batch runner: for each question in a CSV of golden answers generated WITH the knowledge base
(e.g. via LM Studio / LM Notebooks with scraped text attached), fetch the chatbot's answer
via /chat, then judge it against the reference using a symmetric rubric.

This is the counterpart to golden_judge_runner.py which handles the asymmetric case
(golden answers generated WITHOUT context).

Usage
-----
    export PYTHONPATH=src

    # Judge chatbot vs LM-Notebooks golden answers
    python -m evaluation.llm_judge.context_golden_runner \\
        --golden-csv src/evaluation/data/context_golden_answers.csv \\
        --judge-model anthropic/claude-opus-4.7

    # Limit to first 30 questions and resume after interruption
    python -m evaluation.llm_judge.context_golden_runner \\
        --golden-csv src/evaluation/data/context_golden_answers.csv \\
...
Input CSV format
----------------
Required columns: query, golden_answer
Optional columns: gold_url, skip_reason (rows with non-empty skip_reason are skipped)

Output
------
    src/data/feedback/context_golden_results.csv
    Columns: query, gold_url, chatbot_answer, golden_answer,
...
DEFAULT_GOLDEN_CSV = "src/evaluation/data/context_golden_answers.csv"
OUTPUT_CSV = "src/data/feedback/context_golden_results.csv"
...
def _load_golden_rows(path: str, limit: int | None) -> list[dict]:
    rows = []
...
def main() -> int:
    ...
    rows = _load_golden_rows(args.golden_csv, args.limit)
    if not rows:
    ...
    with output_path.open("a", encoding="utf-8", newline="") as f:
        ...
        for i, row in enumerate(rows, start=1):
            ...
            try:
                result = judge_against_context_golden(
                    query,
                    chatbot_answer,
                    row["golden_answer"],
