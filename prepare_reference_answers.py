...
answers_df = pd.read_csv('src/evaluation/data/answers_google_gemini-3.5-flash_t0.2_p2.csv')
...
reference_data = []
with open('src/evaluation/data/final_notebooklm_QA.jsonl', 'r', encoding='utf-8') as f:
    for line in f:
        if line.strip():
...
filtered_reference = [
    item for item in reference_data 
    if item['Pytanie'] in generated_questions
]

print(f"✓ Filtrowane pytania (wspólne): {len(filtered_reference)}")
print(f"  Pytania z answers ale nie w JSONL: {len(generated_questions) - len(filtered_reference)}")

# Konwertuj do DataFrame z kolumnami: pytanie, odpowiedz
ref_df = pd.DataFrame([
    {
        'pytanie': item['Pytanie'],
        'odpowiedz': item['Odpowiedź']
    }
...
print(f"python -m evaluation.text_metrics \\")
print(f"  --generated-csv src/evaluation/data/answers_google_gemini-3.5-flash_t0.2_p2.csv \\")
print(f"  --reference-csv {output_path} \\")
print(f"  --output-dir src/evaluation/results")
